...
class PAJ7620U2(object):
	...
	def play_video(self, command):
		if self.process:
			print(f"Terminating current VLC process...:{self.process}")
			self.process.kill()
			subprocess.Popen("pkill vlc", shell=True)
			self.process.wait()  # Ensure the process is fully terminated
			time.sleep(0.5)  # Add a short delay before starting the next video
		print(f"Starting video: {command}")
	...
	def check_gesture(self):
		...
		if Gesture_Data == PAJ_UP:
			...
			print("Right\r\n")	
			if self.index + 1 < len(self.miku_cycle): 
				self.index+=1
			else: 
				self.index = 0
			self.save()
